...
def generate_train_data(train_file_location, test_file_location, n=943, m=1682):
    train_data = pd.read_csv(
        train_file_location, sep="\t", names=r_cols, encoding="latin-1"
    )
    test_data = pd.read_csv(
        test_file_location, sep="\t", names=r_cols, encoding="latin-1"
    )
    ...
    for i in range(len(user_record_test)):
        rating_matrix_test[
            user_record_test[i] - 1, movie_record_test[i] - 1
        ] = ratings_record_test[i]
